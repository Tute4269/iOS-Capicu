...
class FourPlayersController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    var player_4_scores = [Int]()
    var player_4_total_score = 0
    var player_4_name_text = "Player 4"
    @IBOutlet weak var player_4_name: UILabel!
    @IBOutlet weak var player_4_score: UILabel!
    @IBOutlet weak var player_4_table: UITableView!
        ...
        if sender.view?.tag == 1 {
            ...
            alert.addAction(UIAlertAction(title: "Confirm", style: .default, handler: { [weak alert] (_) in
                if let the_new_score = alert!.textFields?[0].text {
                    ...
                    UserDefaults.standard.set(the_new_score, forKey: "4p_player1name")
                }
            }))
            ...
            alert.addAction(UIAlertAction(title: "Confirm", style: .default, handler: { [weak alert] (_) in
                if let the_new_score = alert!.textFields?[0].text {
                    ...
                    UserDefaults.standard.set(the_new_score, forKey: "4p_player2name")
                }
            }))
            ...
            alert.addAction(UIAlertAction(title: "Confirm", style: .default, handler: { [weak alert] (_) in
                if let the_new_score = alert!.textFields?[0].text {
                    ...
                    UserDefaults.standard.set(the_new_score, forKey: "4p_player3name")
                }
            }))
            ...
            let alert = UIAlertController(title: "Player 4", message: "Set the name of Player 4", preferredStyle: .alert)
            
            alert.addTextField { (new_score) in
                new_score.placeholder = "Player 4 Name"
                new_score.keyboardType = UIKeyboardType.alphabet
                new_score.autocapitalizationType = .words
            }
            ...
            alert.addAction(UIAlertAction(title: "Confirm", style: .default, handler: { [weak alert] (_) in
                if let the_new_score = alert!.textFields?[0].text {
                    self.player_4_name.text = the_new_score
                    UserDefaults.standard.set(the_new_score, forKey: "4p_player4name")
                }
            }))
    ...
    @IBAction func clearTables(_ sender: Any) {
        ...
        alert.addAction(UIAlertAction(title: "Confirm", style: .default, handler: { [weak alert] (_) in
            ...
            UserDefaults.standard.set("Player 1", forKey: "4p_player1name")
            UserDefaults.standard.set([Int](), forKey: "4p_player1scores")
            ...
            UserDefaults.standard.set("Player 2", forKey: "4p_player2name")
            UserDefaults.standard.set([Int](), forKey: "4p_player2scores")
            ...
            UserDefaults.standard.set("Player 3", forKey: "4p_player3name")
            UserDefaults.standard.set([Int](), forKey: "4p_player3scores")
            
            self.player_4_scores.removeAll()
            self.player_4_total_score = 0
            self.player_4_score.text = String(self.player_4_total_score)
            self.player_4_table.reloadData()
            self.player_4_name.text = "Player 4"
            UserDefaults.standard.set("Player 4", forKey: "4p_player4name")
            UserDefaults.standard.set([Int](), forKey: "4p_player4scores")
        }))
        ...
        alert.addAction(UIAlertAction(title: "Confirm", style: .default, handler: { [weak alert] (_) in
            if let the_new_score = alert!.textFields?[0].text {
                if the_new_score == "" {
                    ...
                    UserDefaults.standard.set(self.player_1_scores, forKey: "4p_player1scores")
                }
            }
        }))
        ...
        alert.addAction(UIAlertAction(title: "Confirm", style: .default, handler: { [weak alert] (_) in
            if let the_new_score = alert!.textFields?[0].text {
                if the_new_score == "" {
                    ...
                    UserDefaults.standard.set(self.player_2_scores, forKey: "4p_player2scores")
                }
            }
        }))
        ...
        alert.addAction(UIAlertAction(title: "Confirm", style: .default, handler: { [weak alert] (_) in
            if let the_new_score = alert!.textFields?[0].text {
                if the_new_score == "" {
                    ...
                    UserDefaults.standard.set(self.player_3_scores, forKey: "4p_player3scores")
                }
            }
        }))
    ...
    @IBAction func addToPlayerFour(_ sender: Any) {
        let alert = UIAlertController(title: "Player 4 Wins", message: "Add points to the winner", preferredStyle: .alert)
        ...
        alert.addAction(UIAlertAction(title: "Confirm", style: .default, handler: { [weak alert] (_) in
            if let the_new_score = alert!.textFields?[0].text {
                if the_new_score == "" {
                    ...
                    // append new score to table
                    self.player_4_scores.append(Int(the_new_score)!)
                    
                    // update the total score variable
                    self.player_4_total_score = self.player_4_total_score + (Int(the_new_score)!)
                    
                    self.player_4_table.reloadData()
                    self.player_4_score.text = String(self.player_4_total_score)
                    
                    UserDefaults.standard.set(self.player_4_scores, forKey: "4p_player4scores")
                }
            }
        }))
    ...
    internal func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        } else if tableView.isEqual(player_4_table) {
            rowCount = player_4_scores.count
        }
    ...
    internal func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if tableView.isEqual(player_1_table) {
            ...
            cell.textLabel?.text = String(player_4_scores[indexPath.row])
        }
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        ...
        if editingStyle == UITableViewCellEditingStyle.delete {
            if tableView.isEqual(player_1_table) {
                ...
                UserDefaults.standard.set(player_1_scores, forKey: "4p_player1scores")
            } else if tableView.isEqual(player_2_table) {
                // update list items when deleting item
                ...
                UserDefaults.standard.set(player_2_scores, forKey: "4p_player2scores")
            } else if tableView.isEqual(player_3_table) {
                // update list items when deleting item
                ...
                UserDefaults.standard.set(player_3_scores, forKey: "4p_player3scores")
            } else if tableView.isEqual(player_4_table) {
                // update list items when deleting item
                player_4_total_score -= player_4_scores[indexPath.row]
                player_4_score.text = String(player_4_total_score)
                
                // remove from places Dictionary, set the standardDefault, and reload the table data
                player_4_scores.remove(at: indexPath.row)
                player_4_table.reloadData()
                UserDefaults.standard.set(player_4_scores, forKey: "4p_player4scores")
            }
        }
    }
    ...
    override func viewDidLoad() {
        ...
        let longPressGestureRecognizer4 = UILongPressGestureRecognizer(target: self, action: #selector(ThreePlayersController.handlePress))
        ...
        player_4_name.isUserInteractionEnabled = true
        ...
        player_4_name.addGestureRecognizer(longPressGestureRecognizer4)
        
        let logo = UIImage(named: "domino4.png")! as UIImage
        ...
        if let name_1 = UserDefaults.standard.object(forKey: "4p_player1name") as? String {
            player_1_name.text = name_1
            player_1_name_text = name_1
        }
        
        if let name_2 = UserDefaults.standard.object(forKey: "4p_player2name") as? String {
            player_2_name.text = name_2
            player_2_name_text = name_2
        }
        
        if let name_3 = UserDefaults.standard.object(forKey: "4p_player3name") as? String {
            player_3_name.text = name_3
            player_3_name_text = name_3
        }
        
        if let name_4 = UserDefaults.standard.object(forKey: "4p_player4name") as? String {
            player_4_name.text = name_4
            player_4_name_text = name_4
        }
        
        if let player_1_scores_arr = UserDefaults.standard.object(forKey: "4p_player1scores") as? [Int] {
            player_1_scores = player_1_scores_arr
        }
        
        if let player_2_scores_arr = UserDefaults.standard.object(forKey: "4p_player2scores") as? [Int] {
            player_2_scores = player_2_scores_arr
        }
        
        if let player_3_scores_arr = UserDefaults.standard.object(forKey: "4p_player3scores") as? [Int] {
            player_3_scores = player_3_scores_arr
        }
        
        if let player_4_scores_arr = UserDefaults.standard.object(forKey: "4p_player4scores") as? [Int] {
            player_4_scores = player_4_scores_arr
        }
        ...
        player_3_score.text = String("\(player_3_total_score)")
        player_4_total_score = player_4_scores.reduce(0, { x, y in x + y })
        player_4_score.text = String("\(player_4_total_score)")
    }
